...
const { findByPrefix } = require('../utils/fileHelper');
...
// yt-dlp butuh ffmpeg sendiri untuk merge bestvideo+bestaudio -> mp4.
// Tanpa ini, yt-dlp diam-diam fallback ke system PATH; jika ffmpeg tidak ada di
// PATH, merge gagal (tanpa error fatal) dan file hasil download BUKAN .mp4,
// melainkan format single-stream apa pun yang tersedia (mis. .webm).
// Pakai binary yang sama dengan yang dipakai ffmpeg.service.js agar konsisten.
const FFMPEG_LOCATION = config.binaries.ffmpeg || ffmpegStaticPath;
...
async function downloadVideo(url, videoId, resolution, onProgress) {
  ...
  // JANGAN asumsikan ekstensi selalu .mp4: --merge-output-format hanya berlaku
  // ketika yt-dlp benar-benar melakukan merge. Jika format selector jatuh ke
  // fallback single-stream ("/best[height<=...]"), ekstensi asli (mis. .webm)
  // yang dipakai. Cari file yang benar-benar ada di disk.
  const actualPath = findByPrefix(config.folders.downloads, `${videoId}_${resolution}.`);

  if (!actualPath) {
    throw new AppError(
      'Download selesai tetapi file hasil tidak ditemukan di folder downloads.',
      500,
