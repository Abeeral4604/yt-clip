...
async function processClipJob(jobId) {
  ...
  try {
    ...
    // Cari berdasarkan prefix, bukan asumsi ekstensi .mp4 tetap, karena yt-dlp
    // bisa saja menyimpan hasil sebelumnya dengan ekstensi lain (webm/mkv) jika
    // merge tidak terjadi saat itu.
    const cachedSourcePath = findByPrefix(config.folders.downloads, `${videoId}_${resolution}.`);
    let finalSourcePath = cachedSourcePath;
